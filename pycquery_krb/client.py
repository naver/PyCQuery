...
from pycquery_krb.protocol.asn1_structs import METHOD_DATA, ETYPE_INFO, ETYPE_INFO2, \
...
from pycquery_krb.protocol.errors import KerberosErrorCode, KerberosError
from pycquery_krb.protocol.encryption import Key, _enctype_table, _HMACMD5
from pycquery_krb.protocol.constants import PaDataType, EncryptionType, NAME_TYPE, MESSAGE_TYPE
from pycquery_krb.protocol.structures import AuthenticatorChecksum
...
class KerbrosClient:
	...
	def get_TGS(self, spn_user, override_etype = None, is_linux = False):
		...
		kdc_req_body['sname'] = PrincipalName(
			{'name-type': NAME_TYPE.SRV_INST.value, 'name-string': spn_user.get_principalname()})
		kdc_req_body['till'] = (now + datetime.timedelta(days=1)).replace(microsecond=0)
		kdc_req_body['nonce'] = secrets.randbits(31)
		if override_etype:
			kdc_req_body['etype'] = override_etype
		...
		if is_linux:
			...
			if Enctype.AES256 == self.kerberos_session_key.enctype:
				authenticator_data['cksum'] = Checksum({'cksumtype': Cksumtype.SHA1_AES256, 'checksum': make_checksum(
					Cksumtype.SHA1_AES256, self.kerberos_session_key, 6, KDC_REQ_BODY(kdc_req_body).dump())})
	...
	def construct_apreq(self, tgs, encTGSRepPart, sessionkey, flags = None, seq_number = 0, ap_opts = []):
		...
		ap_req['authenticator'] = EncryptedData(
			{'etype': self.kerberos_cipher_type, 'cipher': authenticator_data_enc, 'kvno': 3})
